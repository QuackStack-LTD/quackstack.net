...
- **HTTPS implementation** for secure access (the server was Windows-based with IIS)
...
Since the system ran on Windows Server with IIS, we configured SSL certificates and set up HTTPS bindings. The infrastructure was already in place — we just enabled the security layer.

## The Real Challenges

The school handled the go-live mid-semesterginally built by another developer, so understanding the existing codebase and ASP.NET Web Forms architecture took time. Limited architectural documentation meant we had to learn the system by reading through code. Working with legacy patterns like code-behind files, ViewState, and postbacks required careful refactoring to improve maintainability without breaking existing functionality.
