				...
				>
					{[...defaultTechnologies, ...defaultTechnologies].map((tech, index) => (
						<motion.div
							...
							className='flex-shrink-0 relative overflow-hidden group px-8 py-4 rounded-full border border-[rgba(var(--duck-rgb),0.18)] hover:border-[rgba(var(--duck-rgb),0.28)] transition-all duration-500 cursor-pointer backdrop-blur-xl bg-[linear-gradient(135deg,rgba(var(--duck-rgb),0.12),rgba(var(--duck-rgb),0.06))] hover:shadow-[0_8px_32px_0_rgba(var(--duck-rgb),0.25)] before:absolute before:inset-0 before:bg-gradient-to-r before:from-[rgba(var(--duck-rgb),0.06)] before:via-transparent before:to-[rgba(var(--duck-rgb),0.06)] before:translate-x-[-100%] hover:before:translate-x-[100%] before:transition-transform before:duration-700 after:absolute after:inset-[1px] after:rounded-full after:bg-gradient-to-br after:from-white/5 after:via-transparent after:to-transparent after:opacity-0 hover:after:opacity-100 after:transition-opacity after:duration-300 flex items-center space-x-3'
							style={{
								backdropFilter: 'blur(16px) saturate(150%)',
								WebkitBackdropFilter: 'blur(16px) saturate(150%)',
								boxShadow: '0 4px 16px 0 rgba(var(--duck-rgb), 0.15), inset 0 1px 0 0 rgba(255, 255, 255, 0.05)',
							}}
						>
							<span className='text-2xl relative z-10 group-hover:drop-shadow-sm transition-all duration-300'>{tech.icon}</span>
							<span className='text-primary font-medium whitespace-nowrap relative z-10 group-hover:drop-shadow-sm transition-all duration-300'>{tech.name}</span>
